    ...
    minimum_support_threshold( minimum_support_threshold )
{
    ...
    // here we are trying to create the tree
    for ( const Transaction& transaction : transactions ) {
        ...
        for ( const auto& pair : items_ordered_by_frequency ) {
            ...
            // isme check kar rahe haii ki item hai ki nhii transaction pta nhii kyu par kar rahe haiii
            if ( std::find( transaction.cbegin(), transaction.cend(), item ) != transaction.cend() ) {
                ...
                // yahan pe yeh dekh rhe haii ki kya humne iska koi bhi node kra haii kya
                ...
                // yahan pe agar humne nhii kra tha toh hume end vala mil jaega toh hume insert karna hoga
                if ( it == curr_fpnode->children.cend() ) {
                    ...
                    // yahan hum naya node insert karte hue
                    curr_fpnode->children.push_back( curr_fpnode_new_child );

                    // update the node-link structure
                    // yahan pe header table se count dekh rahe haii agar greater than 0 then hum last node jaake vahan pe add kar rahe haii
